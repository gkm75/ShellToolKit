...
func hexProcessor(cfg *common.Config, outFile *bufio.Writer, block []byte, count int, pos int64) {
	for _, b := range block {
		fmt.Fprintf(outFile, "%02x", b)
	}
}

func hexProcessorUpper(cfg *common.Config, outFile *bufio.Writer, block []byte, count int, pos int64) {
	for _, b := range block {
		fmt.Fprintf(outFile, "%02X", b)
	}
}
